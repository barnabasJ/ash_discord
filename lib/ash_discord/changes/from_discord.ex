defmodule AshDiscord.Changes.FromDiscord do
  ...
  defp transform_message(changeset, discord_data) do
    ...
    guild_discord_id = Map.get(discord_data, :guild_id)

    # Get channel_discord_id from struct, with fallback to arguments for API fetching
    channel_discord_id = Map.get(discord_data, :channel_id)
  ...
  defp transform_emoji(changeset, discord_data) do
    ...
    case Map.get(discord_data, :name) do
      ...
      _ ->
        ...
        |> Ash.Changeset.force_change_attribute(
          :animated,
          Map.get(discord_data, :animated, false)
        )
  ...
  defp transform_voice_state(changeset, discord_data) do
    ...
    |> Ash.Changeset.force_change_attribute(:session_id, Map.get(discord_data, :session_id))
    |> maybe_set_attribute(:deaf, Map.get(discord_data, :deaf))
    |> maybe_set_attribute(:mute, Map.get(discord_data, :mute))
  ...
  defp transform_message_reaction(changeset, discord_data) do
    ...
    # Set ID fields with struct precedence over arguments
    |> set_message_reaction_id_fields(discord_data)
  end

  # Set all ID fields for message reactions with struct precedence over arguments
  defp set_message_reaction_id_fields(changeset, discord_data) do
    changeset
  ...
  # Helper to set individual ID field with struct precedence over arguments
  # Handles both naming conventions: :user_id vs :user_discord_id automatically
  ...
  # Helper to get ID with struct precedence over arguments
  ...
  defp get_target_field_name(resource, struct_key) do
    # Convert :user_id -> :user_discord_id, handling the naming correctly
  ...
  defp transform_interaction(changeset, discord_data) do
    ...
    |> Ash.Changeset.force_change_attribute(:discord_id, Map.get(discord_data, :id))
    |> Ash.Changeset.force_change_attribute(:type, Map.get(discord_data, :type))
    |> Ash.Changeset.force_change_attribute(:token, Map.get(discord_data, :token))
    |> Ash.Changeset.force_change_attribute(
      :application_id,
      Map.get(discord_data, :application_id)
    )
    ...
    |> maybe_set_attribute(:locale, Map.get(discord_data, :locale))
