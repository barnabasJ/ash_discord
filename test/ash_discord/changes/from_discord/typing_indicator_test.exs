defmodule AshDiscord.Changes.FromDiscord.TypingIndicatorTest do
  ...
  Note: Typing indicators are ephemeral events and not fetchable from Discord API,
  so only the struct-first pattern is applicable (no API fallback).
  """
  ...
  describe "struct-first pattern" do
    ...
    test "creates typing indicator from discord struct with all attributes" do
      ...
      Mimic.expect(Nostrum.Api.Channel, :get, fn ^channel_id ->
        {:ok, channel(%{id: channel_id, name: "test-channel", type: 0, guild_id: guild_id})}
      end)
      ...
      assert created.timestamp == DateTime.from_unix!(typing_struct.timestamp, :second)
    ...
    test "handles DM typing indicator without guild_id" do
      ...
      Mimic.expect(Nostrum.Api.Channel, :get, fn ^channel_id ->
        {:ok, channel(%{id: channel_id, name: "dm-channel", type: 1, guild_id: nil})}
      end)
    ...
    test "handles recent typing indicator" do
      ...
      assert created.channel.discord_id == channel_id
      assert created.guild.discord_id == guild_id
      assert created.timestamp == DateTime.from_unix!(current_timestamp, :second)
    end
    ...
    test "handles old typing indicator" do
      ...
      assert created.channel.discord_id == channel_id
      assert created.guild.discord_id == guild_id
      assert created.timestamp == DateTime.from_unix!(old_timestamp, :second)
    end
    ...
    test "handles typing indicator in voice channel" do
      ...
      Mimic.expect(Nostrum.Api.Channel, :get, fn ^channel_id ->
        {:ok, channel(%{id: channel_id, name: "voice-channel", type: 2, guild_id: guild_id})}
      end)
    ...
    test "handles typing indicator in thread" do
      ...
      Mimic.expect(Nostrum.Api.Channel, :get, fn ^channel_id ->
        {:ok, channel(%{id: channel_id, name: "thread", type: 11, guild_id: guild_id})}
      end)
  ...
  describe "upsert behavior" do
    ...
    test "updates existing typing indicator instead of creating duplicate" do
      ...
      assert updated.user.discord_id == original.user.discord_id
      assert updated.channel.discord_id == original.channel.discord_id
      assert updated.guild.discord_id == original.guild.discord_id
      assert updated.timestamp == DateTime.from_unix!(1_673_788_200, :second)
    end
    ...
    test "upsert works with guild context changes" do
      ...
      Mimic.expect(Nostrum.Api.Channel, :get, fn ^channel_id ->
        {:ok,
         channel(%{id: channel_id, name: "test-channel", type: 0, guild_id: initial_guild_id})}
      end)
      ...
      assert updated.channel.discord_id == channel_id
      assert updated.guild.discord_id == updated_guild_id
      assert updated.timestamp == DateTime.from_unix!(1_673_788_200, :second)
    end
  end
end
