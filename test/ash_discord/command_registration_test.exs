defmodule AshDiscord.CommandRegistrationTest do
  ...
  describe "command scope filtering" do
    test "filters global commands correctly" do
      commands = [
        %Command{name: :global_help, scope: :global, description: "Global help", type: :chat_input, options: [], domain: TestDomain, resource: TestResource, action: :help},
        %Command{name: :guild_admin, scope: :guild, description: "Guild admin", type: :chat_input, options: [], domain: TestDomain, resource: TestResource, action: :admin},
        %Command{name: :guild_mod, scope: :guild, description: "Guild mod", type: :chat_input, options: [], domain: TestDomain, resource: TestResource, action: :moderate}
      ]
  ...
  describe "to_discord_command conversion" do
    test "converts command struct to Discord API format" do
      ...
      assert discord_command.type == 1  # chat_input type
  ...
  describe "mocked registration calls" do
    ...
    test "guild command registration API call format" do
      ...
      expect(Nostrum.Api.ApplicationCommand, :bulk_overwrite_guild_commands, fn received_guild_id, commands ->
  ...
  describe "CommandFilter behavior" do
    ...
    test "filter works correctly with guild context" do
      ...
      large_guild = %{id: 123456789, name: "Large Guild"}
  ...
  describe "unknown command handling" do
    ...
    defmodule UnknownCommandTestConsumer do
      use AshDiscord.Consumer, domains: []
    ...
    test "handles unknown command interaction gracefully" do
      ...
      expect(Nostrum.Api.Interaction, :create_response, fn interaction_id, token, response ->
        ...
        assert token == "interaction_token"
      ...
      interaction = %{
        ...
        data: %{
          name: "nonexistent_command",  # This command doesn't exist in our registry
          type: 1
        },
        user: %{id: "user_123"},
        guild_id: "guild_456"
      ...
      # Should handle the unknown command gracefully (returns API response)
      assert {:ok, _} = result
